...
async function main() {
    ...
    const priceFeedEthDeploy = await aggreagatorV3.deploy(ethers.parseEther("100"))
    ...
    await tx.wait()
    tx = await nftAuction.connect(buyer).placeBid(0, ethers.parseEther("1"), UsdcAddress);
    await tx.wait()

    // ETH参与竞价
    tx = await nftAuction.connect(buyer).placeBid(0, ethers.parseEther("0.02"), ethers.ZeroAddress, { value: ethers.parseEther("0.01") });
    await tx.wait()
